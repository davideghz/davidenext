  ...
  return (
    <div className="pt-16 pb-20 sm:pt-20">
      <div className="mx-auto max-w-2xl xl:relative">
        <Link href="/" aria-label="Go back to home" className={backButtonClass}>
        ...
        <div className="mt-16 sm:mt-20 flex flex-col space-y-16">
          {posts.map((post) => (
            <article
              key={post.id}
                ...
                {post.data.tags && post.data.tags.length > 0 && (
                  <div className="relative z-10 mt-2 flex flex-wrap gap-1.5">
                    {post.data.tags.map((t: { tag: string | null }, i: number) => (
                      <span key={i} className="text-xs px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-500 dark:bg-indigo-500/10 dark:text-indigo-400">
                        {t.tag}
                      </span>
                    ))}
                  </div>
